...
void placar() {
    ...
    FILE *R_Vitorias = fopen("vitorias.txt", "r");
    char nome[6];
    int vitorias;

    if(R_Vitorias == NULL) {
        printf("\nErro ao mostrar o placar");
    }

    while(fscanf(R_Vitorias, "%s %d", nome, &vitorias) != EOF) {
        printf("Nome: %s        %d\n", nome, vitorias);
    }
...
int main() {
    ...
    FILE *W_Vitorias = fopen("vitorias.txt", "w");
