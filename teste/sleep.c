#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
#include <stdbool.h>
#include <unistd.h>

int main()
{
    int decisionTable[9][3] = { // Tesoura = 1, Papel = 2, Pedra = 3, Empate = 0
        1, 1, 0,
        1, 2, 1,
        1, 3, 3,
        2, 1, 1,
        2, 2, 0,
        2, 3, 2,
        3, 1, 3,
        3, 2, 2,
        3, 3, 0,
    };
    
    char jogadas[4][8] = {
        "empate",
        "tesoura",
        "papel",
        "pedra",
    };
    
    int i = 0, jogadaNumeral;
    char jogada[8], decisao;
    bool endGame = false;
    srand(time(NULL)); 
    
    do {
        printf("\nQual a sua jogada? (tesoura, papel, pedra)\n");
        scanf("%s", jogada);
        
        int max = 3, min = 1;
        int jogadaCPU = rand() % (max - min + 1) + min;
        
        if(strcmp(jogada, "tesoura") == 0) {
            jogadaNumeral = 1;
        } else if(strcmp(jogada, "papel") == 0) {
            jogadaNumeral = 2;
        } else if(strcmp(jogada, "pedra") == 0) {
            jogadaNumeral = 3;
        } else {
            printf("\nJogada inválida.");
            continue;
        }
        
        for(i = 0; i < 9; i++) {
            if(decisionTable[i][0] == jogadaNumeral && decisionTable[i][1] == jogadaCPU) {
                int resultado = decisionTable[i][2];
                printf("\nSua jogada: %s\nCPU: %s\n", jogadas[jogadaNumeral], jogadas[jogadaCPU]);
                
                if(resultado == jogadaNumeral) {
                    printf("Parabéns. Você ganhou!\n");
                } else if(resultado == jogadaCPU) {
                    printf("Você perdeu!\n");
                } else {
                    printf("Empate. Jogue novamente!\n");
                    break;
                }
                
                printf("\nVocê deseja jogar novamente? [y/n]\n> ");
                scanf(" %c", &decisao); 

                if(decisao == 'n' || decisao == 'N') {
                    endGame = true;
                    printf("Obrigado pela partida!\n");
                } 
                if(decisao == 'y' || decisao == 'Y') {
                    printf("\nIniciando novo jogo...\n");
                    sleep(3);
                    system("clear");
                }
                
                break;
            }
        }

    } while (!endGame);

    return 0;
}
